...
namespace Emp_Mvc_Client.Customclass
{
    ...
    public sealed class ValidBirthDate : ValidationAttribute
    {
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            DateTime mindt = Convert.ToDateTime("12/01/1995");
            DateTime maxdt = Convert.ToDateTime("12/06/2005");
    ...
    public sealed class ValidJoiningDate : ValidationAttribute
    {
        ...
        {
            DateTime entered_dob = Convert.ToDateTime(value);
            DateTime mindt = Convert.ToDateTime("01/25/2010");
            DateTime maxdt = Convert.ToDateTime("12/6/2022");
            if (entered_dob >= mindt && entered_dob <= maxdt)
                return ValidationResult.Success;
            else
                return new ValidationResult(ErrorMessage);
    ...
    public class DeptValidation : ValidationAttribute
    {
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            if (Convert.ToString(value) == "1" || Convert.ToString(value) == "2" ||
                Convert.ToString(value) == "3" || Convert.ToString(value) == "4" ||
                Convert.ToString(value) == "5")
                return ValidationResult.Success;
            else
                return new ValidationResult(ErrorMessage);
        }
